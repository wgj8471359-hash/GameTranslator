...
| 适用场景 | 解决痛点 | 实际体验 |
| :--- | :--- | :--- |
| **🎮 外语游戏 (RPG / 视觉小说)** | 游戏画面保持动态渲染，不打断 Live2D 与背景音效；人名框与对话框自动分离开；支持实时模式自动跟读台词。 | 原文位置浮现半透明圆角气泡，点一下切原文，长按可单句重译。 |
| **📚 生肉漫画 / 电子书** | 日漫纵排横排多行台词分散，传统 OCR 乱序拼词。 | 并查集 2D 几何聚类合并为完整对白，流畅自然。 |
| **📱 海外社交与网页浏览 (X / Reddit / 网页)** | 复制多段文字跳转翻译软件繁琐；滑动页面时文字易错位。 | 单击即显；实时模式下支持运动学平移跟踪，气泡跟随页面滚动原地平移，触控穿透不影响翻页浏览。 |
...
- 📱 **真·手机端完全离线闭环 (Termux + llama.cpp)**：
  - 深度优化**手机本机 Termux + llama.cpp 端侧直连**方案（直接监听 `http://127.0.0.1:8080/v1`）。
  - 无需电脑、不占局域网、不耗流量，真正做到离线即开即用。
- 🔄 **实时差分翻译与运动学平移跟踪**：
  - 支持常驻后台实时差分监听（长按悬浮球切换模式，物理帧率自适应轮询采样）；
  - 基于空间 IoU、编辑距离与消抖窗口，未变动文本零闪烁零重绘，消失文本平滑淡出；
  - 网页或条漫上下滚动时，运动学平移跟踪（Kinematic Tracker）使气泡原地跟随位移，无需重复请求模型；
  - 内置多维自截屏光学回影免疫，有效隔绝自身半透明气泡导致的识别干扰。
- 🔒 **零审查与隐私安全**：
  - OCR 基于 Google ML Kit，纯本地离线推理，绝不上报截图数据；
  - 翻译直连私有部署的大模型（如 Tencent Hunyuan-MT2、Qwen 等），无敏感词过滤，保留完整表达。
...
- 💬 **轻量级局部自适应浮层与行间距避让**：
  ...
  - 全景注册文本框包围盒，自适应计算气泡最大高度与下行物理间隔，杜绝上下行气泡重叠遮挡。
- ⚡ **触控穿透与多维手势交互**：
  - 实时模式自动启用 `FLAG_NOT_TOUCHABLE` 沉浸穿透，滑动页面、操作游戏无阻碍；
  - **双向切换**：轻触单个气泡可在「译文」与「原文」间秒切；
  - **单句重译**：长按单个气泡强制清空该句缓存并即时触发单句重译；
  - **长文本滚动**：译文超出限制时支持在气泡内垂直拖拽滚动；
  - 单次模式下轻触屏幕任意空白区域立即瞬间撤销所有浮层。
- 🧠 **本地 LRU 翻译记忆缓存**：
  - 内存维护 500 条容量的 LRU 缓存，配合标点与空白归一化消抖；
  - 重复出现的台词与固定 UI 按钮瞬间命中秒显，节省推理开销。
- ⚡ **双形态流式逐句上屏**：
...
- 🌓 **纯原生暗/白双主题无缝适配**：
  - 深度跟随系统深色/浅色模式，状态栏与导航栏一体化沉浸，无硬编码色值泄露。
...
| 手势 | 作用对象 | 功能描述 |
| :--- | :--- | :--- |
| **单击** | 悬浮球 | 单次截屏翻译模式下触发全屏静默截屏、OCR、聚类与翻译 |
| **长按** | 悬浮球 | 在「单次截屏模式」与「实时差分模式」之间切换（实时开启时悬浮球常驻指示灯） |
| **双击** | 悬浮球 | 快速调出 App 主配置设置界面 |
| **拖动** | 悬浮球 | 自由拖拽至屏幕任意位置，松手后自动平滑贴靠最近侧屏幕边缘 |
| **轻触气泡** | 任意翻译气泡 | 在「中文译文」与「原文字幕」之间即时来回切换 |
| **长按气泡** | 任意翻译气泡 | 强制清除该句本地缓存，即时触发单句重新翻译 |
| **气泡内滑动** | 任意长文本气泡 | 译文超出原框高度限制时，可在气泡内平滑上下拖拽滚动浏览完整内容 |
| **轻触空白处** | 任意非气泡区域 | 仅在单次模式下生效，立即移除所有悬浮气泡，释放触控 |
...
### 3. 悬浮球与气泡微调
- **悬浮球大小**：默认 `44dp`，不遮挡画面。
- **气泡不透明度**：默认 `85%`（深色半透明，文字对比度高）。
- **气泡字号自适应**：默认 `8sp ~ 16sp`，根据原文字矩形物理尺寸自动缩放。

### 4. 实时差分与缓存配置
- **实时采样间隔**：默认 `1200ms`（画面静止时自适应降频至 `2500ms`，平衡响应速度与设备发热功耗）。
- **翻译记忆缓存**：内存维护 500 条容量的 LRU 缓存，配合标点与空格归一化模糊容错。
